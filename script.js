...
(function formularioContato() {
  ...
  const mediaPrefereReducao = window.matchMedia('(prefers-reduced-motion: reduce)');

  function deslocarParaElemento(elemento) {
    if (!elemento) return;
    elemento.scrollIntoView({
      behavior: mediaPrefereReducao.matches ? 'auto' : 'smooth',
      block: 'center'
    });
  }
  ...
  function validarSegmento() {
    ...
    if (checkboxSegmento.length === 0) return true;
    
    const algumMarcado = Array.from(checkboxSegmento).some(cb => cb.checked);
    const containerSegmento = form.querySelector('#segmento-error');
  ...
  function validarTermos() {
    ...
    const containerTermos = form.querySelector('#termos-error');
  ...
  form.addEventListener('submit', async function(e) {
    ...
    if (!validarFormulario()) {
      ...
      if (primeiroErro) {
        primeiroErro.focus();
        deslocarParaElemento(primeiroErro);
      }
      return;
    }
  ...
  function mostrarMensagemSucesso() {
    ...
    if (mensagemSucesso) {
      ...
      deslocarParaElemento(mensagemSucesso);
